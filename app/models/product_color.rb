class ProductColor < ApplicationRecord
  ...
  validates :color_id, presence: false
  ...
  scope :best_sell, -> {find_by_sql("select p.product_id, sum(od.quantity)as quantity from product_colors as p inner join order_items as od on p.id = od.product_colors_id group by  p.product_id order by sum(od.quantity) desc")}
end
