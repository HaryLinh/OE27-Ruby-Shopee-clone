class Users::OrdersController < ApplicationController
  ...
  def params_order
   ...
   session[:order].values.to_a.each do |order|
    item << {quantity: order["quantity"], product_color_id: order["product_color"], product_id: order["product_color"], price_product: order["price"], amount: (order["quantity"].to_i * order["price"].to_f)}
   end
